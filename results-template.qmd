...
```{r}
#summary_mean_cor <- df_main_fill %>% group_by(subj) %>% summarize(mean_is_correct = mean(isCorrect))
df_main_fill$leg <- ifelse(df_main_fill$isLeg == "leg",0.5,-0.5)
# # leg = 0.5 , ill = -0.5

df_main_fill$sim <- ifelse(df_main_fill$sent == 1 | df_main_fill$sent == 2 | df_main_fill$sent == 5 | df_main_fill$sent == 6 | df_main_fill$sent == 9 | df_main_fill$sent == 12 | df_main_fill$sent == 16 | df_main_fill$sent == 17 | df_main_fill$sent == 18 | df_main_fill$sent == 20, 0.5,-0.5)
# # sim = 0.5, dis = -0.5

summary_leg_sim <- df_main_fill %>% group_by(leg, sim, subj) %>% summarize(mean_is_correct = mean(isCorrect)) %>% group_by(leg, sim) %>% summarize(mean_is_correct = mean(isCorrect))
summary_leg_sim
```
