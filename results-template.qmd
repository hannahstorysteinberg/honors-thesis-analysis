...
When participants read illegible sentences, they were `r sign_b1_interp` likely to change the sentence (b = `r beta1_est`,se = `r se_b1`), and this effect `r p_val_b1_sig_interp` statistically significant (z = `r z_b1`, p = `r p_val_b1`). This effect was allowed to vary across participants ($\tau_{11}$ = `r t11_est`) 

### Hypothesis 2: effect of similarity

When participants read similar sentences, they were `r sign_b2_interp` likely to change the sentence (b = `r beta2_est`,se = `r se_b2`), and this effect `r p_val_b2_sig_interp` statistically significant (z = `r z_b2`, p = `r p_val_b2`). This effect was allowed to vary across participants ($\tau_{22}$ = `r t22_est`) 
...
When the letter is similar, people are `r sign_bsim_interp` likely to change an illegible sentence compared to a legible sentence (b = `r betasim_est`,se = `r se_bsim`), and this effect `r p_val_bsim_sig_interp` statistically significant (z = `r z_bsim`, p = `r p_val_bsim`).

When the letter is dissimilar, people are `r sign_bdis_interp` likely to change an illegible sentence compared to a legible sentence (b = `r betadis_est`,se = `r se_bdis`), and this effect `r p_val_bdis_sig_interp` statistically significant (z = `r z_bdis`, p = `r p_val_bdis`).

The effect of legibility was , when the letters were similar, compared to dissimilar 
...
plot_df <- summary_df %>%
  mutate(
    legibility = factor(leg, levels = c(1, 0), labels = c("Legible", "Illegible")),
    condition  = factor(sim, levels = c(1, 0), labels = c("Similar", "Dissimilar"))
  )
