...
# test edit distance function (adist(x,y)) (done)
# filler accuracy = correct if edit distance less than or equal to 2
# remove people with filler accuracy less than 90%
# remove people who learned english after the age of 7 (8 and above)
# change classification of critical sentences
# remove all punctuation (done)
# remove ii in richard (done)
...
df <- filter(df, !(parameter == "_PreloadFailed_")) # remove trials where the photos didn't load
...
df_main_fill <- df_main_fill %>% mutate(isCorrect = as.numeric(adist(value,text)<3))
